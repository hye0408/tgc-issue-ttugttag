...
export function getSettings() {
  return {
    ...
    jiraVocTypeValue: process.env.JIRA_VOC_TYPE?.trim() || "VOC",
    jiraBugTypeValue: process.env.JIRA_BUG_TYPE?.trim() || "버그",
    defaultTestUrl:
...
export function ensureJiraSettings(settings) {
  ["jiraUrl", "jiraEmail", "jiraApiToken", "jiraProjectKey", "jiraIssueType"].forEach(
    (key) => {
      if (!settings[key]) {
  ...
  if (settings.jiraIssueType.toUpperCase() !== "VOC") {
    throw new Error("JIRA_ISSUE_TYPE 값은 VOC로 고정되어야 한다.");
  }
}
...
export function sanitizeIssue(issue, uploadedFilenames = []) {
  ...
  const kind = normalizedKind === "VOC" ? "VOC" : "BUG";

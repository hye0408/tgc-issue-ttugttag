...
async function createJiraIssue(issue, settings) {
  if (!issue.customerName || issue.customerName === "추가 확인 필요") {
  ...
  const fields = {
    ...
    issuetype: parseIdOrJson(settings.jiraIssueType),
    description: buildIssueAdf(issue, settings),
  };

  fields[settings.jiraCustomerFieldId] = issue.customerName;
  fields[settings.jiraVocTypeFieldId] = {
    id: getVocTypeOptionId(settings, issue.vocType),
  };

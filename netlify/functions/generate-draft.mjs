...
export default async function handler(request) {
  ...
  try {
    ...
    if (!String(payload.customerName || "").trim()) {
      return withCors(jsonResponse(400, { error: "고객사명은 필수 입력값이다." }));
    }
        ...
        sanitizeIssue(
          {
            ...
            vocType: issue.voc_type,
            summary: buildSummary(issue.issue_kind, issue.voc_type, issue.menu_path, issue.summary),
            customerName,
...
function buildSummary(issueKind, vocType, menuPath, detailSummary) {
  const isDefect = String(vocType || "").toUpperCase() === "DEFECT" || issueKind === "BUG";
  const prefix = isDefect ? "[PR] 에이전트" : "[VOC][PR] 에이전트";
